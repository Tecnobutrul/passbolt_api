...
echo "\n";
echo " Passbolt Cloud API Admin - Cleanup organizations\n";
echo "\n";
...
if (empty($organizations)) {
    echo "No organization to cleanup.\n";
    exit;
}

foreach ($organizations as $organization) {
    if (!preg_match('/^[a-z0-9]+[a-z0-9\-_]*[a-z0-9]+$/i', $organization->slug)) {
        echo "Cannot cleanup the organization {$organization->slug}. The organization slug is not valid.\n";
        continue;
    }

    $slugParam = escapeshellarg($organization->slug);
    $command = __DIR__ . "/../cake passbolt cleanup --org={$slugParam}";
    echo "Cleanup: {$organization->slug}.\n";
